...
typedef struct {
    ...
    long   n;       // usato per -R -r // numero di messaggi che il client si aspetta di ricevere dagli altri client prima di eseguire un successivo comando
} operation_t;

/* -------------------- globali -------------------------- */
// array di messaggi ricevuti ma non ancora gestiti
static message_t  *MSGS = NULL;
static const int   msgbatch = 100; // fattore incremento array MSGS
static size_t      msgcur=0; // indice prima posizione libera MSGS
static size_t      msglen=0; // lunghezza attuale array MSGS
/* ------------------------------------------------------- */
...
static int downloadFile(int connfd, char *filename, char *sender) {
    ...
    setData(&msg.data, "", filename, strlen(filename)+1); // il receiver è settato a "" poiché sto facendo un'operazione di download
	...
	switch(msg.hdr.op) {
	case OP_OK: { // il server mi sta rispondendo con il file richiesto
...
static int execute_requestreply(int connfd, operation_t *o) {
    ...
    if (mappedfile) { // avevo richiesto un POSTFILE_OP quindi devo inviare il file
	message_data_t data;
	setData(&data, "", mappedfile, o->size); // receiver è vuoto dato che il server già lo conosce dal messaggio precedente (riga 144)
	if (sendData(connfd, &data) == -1) { // invio il contenuto del file, l'header già lo conosce
	    perror("sending data");
...
int main(int argc, char *argv[]) {
    ...
    int optc;
    char *spath = NULL, *nick = NULL; //spath è il path della socket

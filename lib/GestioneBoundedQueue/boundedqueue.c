...
static void LockQueue(BQueue_t *q)          { pthread_mutex_lock(&q->m);   }
static void UnlockQueue(BQueue_t *q)        { pthread_mutex_unlock(&q->m); }
static void WaitToProduce(BQueue_t *q)      { pthread_cond_wait(&q->cfull, &q->m); }
static void WaitToConsume(BQueue_t *q)      { pthread_cond_wait(&q->cempty, &q->m); }
static void SignalProducer(BQueue_t *q)     { pthread_cond_signal(&q->cfull); }
static void SignalConsumer(BQueue_t *q)     { pthread_cond_signal(&q->cempty); }
